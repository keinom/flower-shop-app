...
export async function updateCustomerOrder(formData: FormData) {
  ...
  if (order.status !== "受付") {
    redirect(`/customer/orders/${orderId}?error=` + encodeURIComponent("受付完了以降は編集できません"));
  }
  ...
  if (!deliveryName) {
    redirect(`/customer/orders/${orderId}/edit?error=` + encodeURIComponent("お届け先名は必須です"));
  }
  ...
  if (orderItems.length === 0) {
    redirect(`/customer/orders/${orderId}/edit?error=` + encodeURIComponent("商品を1つ以上入力してください"));
  }
  ...
  if (updateError) {
    redirect(`/customer/orders/${orderId}/edit?error=` + encodeURIComponent("注文の更新に失敗しました"));
  }
