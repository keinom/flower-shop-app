...
export async function updateRecurringTemplate(formData: FormData) {
  ...
  if (!title) {
    redirect(
      `/admin/recurring/${templateId}/edit?error=` +
        encodeURIComponent("管理名称を入力してください")
    );
  }
  if (!customerId) {
    redirect(
      `/admin/recurring/${templateId}/edit?error=` +
        encodeURIComponent("顧客を選択してください")
    );
  }
  ...
  const monthlyDay = formData.get("monthly_day")
    ? parseInt(formData.get("monthly_day") as string)
    : null;
  const monthlyWeek = formData.get("monthly_week")
    ? parseInt(formData.get("monthly_week") as string)
    : null;
  const monthlyWeekday = formData.get("monthly_weekday")
    ? parseInt(formData.get("monthly_weekday") as string)
    : null;
  const intervalDays = formData.get("interval_days")
    ? parseInt(formData.get("interval_days") as string)
    : null;
  ...
  const deliveryPhone =
    (formData.get("delivery_phone") as string)?.trim() || null;
  const deliveryEmail =
    (formData.get("delivery_email") as string)?.trim() || null;
  const deliveryTimeStart =
    (formData.get("delivery_time_start") as string) || null;
  const deliveryTimeEnd =
    (formData.get("delivery_time_end") as string) || null;
  const purpose = (formData.get("purpose") as string)?.trim() || null;
  const messageCard =
    (formData.get("message_card") as string)?.trim() || null;
  const remarks = (formData.get("remarks") as string)?.trim() || null;

  if (!deliveryName) {
    redirect(
      `/admin/recurring/${templateId}/edit?error=` +
        encodeURIComponent("お届け先名を入力してください")
    );
  }
  ...
  const { error: tmplErr } = await supabase
    ...
    .update({
      ...
      monthly_weekday:
        recurrenceType === "monthly_weekday" ? monthlyWeekday : null,
  ...
  if (tmplErr) {
    redirect(
      `/admin/recurring/${templateId}/edit?error=` +
        encodeURIComponent(
          "テンプレートの更新に失敗しました: " + (tmplErr.message ?? "")
        )
    );
  }
  ...
  redirect(
    `/admin/recurring/${templateId}?success=` +
      encodeURIComponent("テンプレートを更新しました")
  );
}
